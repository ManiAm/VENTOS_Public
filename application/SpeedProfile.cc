...
void SpeedProfile::Change()
{
    ...
    if(mode == 0)
    {
        manager->commandSetSpeed(profileVehicle, normalSpeed);
    }
    else if(mode == 1)
    {
    ...
    else if(mode == 3)
    {
        ...
        {
            manager->commandSetMaxAccel(profileVehicle, 1.5);
            manager->commandSetMaxDecel(profileVehicle, 2.);
        }
...
void SpeedProfile::AccelDecel(double startT, double minV, double maxV)
{
    ...
    {
        manager->commandSetSpeed(profileVehicle, maxV);
        return;
    }

    double v = manager->commandGetVehicleSpeed(profileVehicle);
    ...
    else
    {
        if(v == maxV)
        {
            ...
            {
                manager->commandSetSpeed(profileVehicle, minV);
            }
        }
        else if(v == minV)
        {
...
void SpeedProfile::AccelDecelPeriodic(double startT, double offset, double A, double w)
{
    ...
    {
        manager->commandSetSpeed(profileVehicle, offset);
    }
    ...
    manager->commandSetSpeed(profileVehicle, newSpeed);
}
...
void SpeedProfile::ExTrajectory(double startT)
{
    ...
    {
        manager->commandSetSpeed(profileVehicle, 13.86);
    }
    ...
    manager->commandSetSpeed(profileVehicle, atof(line));
}
